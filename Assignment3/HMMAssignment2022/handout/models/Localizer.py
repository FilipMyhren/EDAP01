...
class Localizer:
    ...
    # add your simulator and filter here, for example    
        ...
        self.__HMM = RobotSimAndFilter.HMMFilter(self.__om, self.__tm, self.__probs)
    #
    ...
    def update(self) -> (bool, int, int, int, int, int, int, int, int, np.array(1)) :
        ...
        self.__trueState = self.__rs.move(self.__trueState)
        ...
        if reading >= len(readings)-1:
            ...
            self.__estimate = self.__sm.reading_to_position(reading)
        self.__probs = self.__HMM.forward(self.__sense)
        self.__estimate = self.__sm.state_to_position(np.argmax(self.__probs))
        #self.__estimate = self.__sm.reading_to_position(random.randrange(self.__sm.get_num_of_states()/4))
